...
% Theorem 119
\begin{theorem}[119]
If $x$ is an ordinal then $x = \{y : y \in R$ and $y < x\}$.
\end{theorem}
...
% Theorem 123
\begin{theorem}[123]
If $x \in R$ then $x + 1$ minimizes $\{y : y \in R$ and $x < y\}$ in $E$.
\end{theorem}
...
and for each $y \in \domain (f | x)$ $(f | x)(y) = f(y)$.
\end{theorem}

% Theorem 127
\begin{theorem}[127]
If $f$ is a map such that $\domain f$ is an ordinal and for each $u \in \domain f$ $f(u) = g(f | u)$
and $h$ is a map such that $\domain h$ is an ordinal and for each $v \in \domain h$ $h(u) = g(h | u)$
